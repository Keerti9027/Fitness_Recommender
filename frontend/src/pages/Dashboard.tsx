...
// import { todoAPI, healthMetricsAPI } from '../lib/api';
// import { Todo, HealthMetrics } from '../lib/localStorage';
// import { Activity, Target, Heart, Droplets, TrendingUp, Plus, CheckCircle, Trash2 } from 'lucide-react';
...
//       const todosArray = Array.isArray(response) ? response : (response.todos || []);
//       setTodos(todosArray);
//     } catch (error) {
...
//     if (user) {
//       const userMetrics = healthMetricsAPI.get(user.id);
//       const todayMetrics = userMetrics
...
//       const newTodoItem = response.todo || response;
...
//       if (todoToUpdate) {
//         const response = await todoAPI.update(id, { completed: !todoToUpdate.completed });
//         // Handle both direct object and nested object responses
...
//     if (user) {
//       healthMetricsAPI.save(updatedMetrics);
//     }
//   };
...
import React, { useState, useEffect } from 'react';
import { useAuth } from '../contexts/AuthContext';
import { Activity as ActivityIcon, TrendingUp, Apple, CheckSquare, Plus, Calendar, X, Heart, Droplets, Footprints } from 'lucide-react';
import { Link } from 'react-router-dom';
...
export default function Dashboard() {
  ...
  const deleteTodo = (id: string) => {
    if (!user) return;
    const allTodos = storage.getTodos(user.id);
    const filteredTodos = allTodos.filter(todo => todo.id !== id);
    // Update storage by removing the deleted todo
    const storageTodos = JSON.parse(localStorage.getItem('fitness_tracker_todos') || '[]') as Todo[];
    const updatedStorageTodos = storageTodos.filter(todo => todo.id !== id);
    localStorage.setItem('fitness_tracker_todos', JSON.stringify(updatedStorageTodos));
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <Link to="/workout-plans" className="block">
          <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
            <ActivityIcon className="h-8 w-8 text-indigo-600 mb-4" />
            <h2 className="text-xl font-semibold mb-2">💪 Workout Plans</h2>
            <p className="text-gray-600">View and manage your workout routines</p>
          </div>
        </Link>
