...
export default function Profile() {
  const { user } = useAuth();
  const [profile, setProfile] = useState(user ? profileAPI.get(user.id) : undefined);
  ...
  const handleSaveProfile = () => {
    ...
    profileAPI.save(user.id, updatedProfile);
              ...
              <div className="space-y-4">
                {isEditing ? (
                  <>
                    ...
                    <div className="mb-4">
                      <label className="block text-gray-700">Water Level (ml)</label>
                      <input
                        type="number"
                        value={waterLevel}
